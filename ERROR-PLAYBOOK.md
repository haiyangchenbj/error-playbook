...
> ⛔ **覆盖防线（2026-09-26 事故后加，读本文件前必读）**
> 本文件是 **canonical（唯一权威）**。`github/error-playbook/`（公开脱敏镜像）与 `github/error-playbook-private/`（私有镜像）
> 都是**下游副本**，内容恒为本文件的子集或脱敏改写版。**同步方向只能是「本文件 → 镜像」，任何情况下禁止用镜像覆盖本文件。**
> 实证：2026-09-26 23:35 镜像同步把滞后副本推回本路径（108,280 → 179,683 B），本地当日沉淀静默消失、无告警，并丢掉
> 本文件独有的 8 个条目（抢救与重编记录见 §7）。**判据：任何写本文件的动作，前后各取一次 `mtime + size` 指纹；
> 体量变小 = 出事；收尾必须核对全文体量，只回验「我插入的串在不在」查不出整文件覆盖。** 详见 §1 第 31 条与 §2.2 W13。
...
**入口 skill 只有一个**：`workflow-guard-rails`（`error-playbook` 已改为薄转发，不在此维护条目）。
...
| 9 | 自写校验脚本有 bug（**两个方向**：对正确文件假报 FAIL，或**静默不检查**报 PASS） | 8+ | 校验脚本先在 known-good 样本上跑通；FAIL 先按脚本 bug 查（2026-09-21 第 4 次：正则锚点漏了 `## ` 前缀；**第 5 次：基线常量硬编码成上一轮的存量值**，见 §3.2.1 V3b；**第 6 次：门禁判据比规则本体更严，合法输入被拒**，见 §3.2.1 V3c；**第 7 次：必填 argv 缺失 → 空跑 exit 0 零输出**，见 §3.2.4；**第 8 次：断言取块时把标题行当正文** —— 「3点」块数字检查用 `re.search(r'\d')` 扫整块，标题 `**今日最重要的3点：**` 里的「3」被误命中 → 对着正确文件假报 `has_digit=True`，见 §2.8 M5 第 3 条；**第 9 次（2026-09-26）：断言/取值按「记忆里的键名」写而非按真源结构写** —— 判据写 `count_key(profile, 'evidence')`，而真源键名是 `evidence_log`（`evidence` 只是 `stable_preferences[]` 里的一个字符串字段名）→ 取回 `None`，在正确文件上报 `PROFILE evidence = 95  =None` FAIL。**同族第二形态：本条判据够不到目标 = 静默不检查，只是这次恰好断言了 `== 95` 才暴露**；若当时写的是 `ev is not None` 或直接打印，就会安静地空跑过去。门禁追加：**写取值/断言前先 `print(list(真源.keys()))` 或 walk 一遍结构，禁按记忆写键名**） | V1/V3c/V7 |
...
| 29 | **`%` 格式化含中文标点的串 → `ValueError: unsupported format character '?' (0xff09 / 0x3001)`**：全角括号 `（）`、顿号 `、` 被当成格式说明符；千分位 `%,.0f` 同样炸。**同一坑按轮次反复出现**（9/14、9/17、9/24 各一次） | 3+ | 含中文或中文标点的格式化串**一律用 f-string**；禁对这类串用 `%`、禁与 `.format()` 混用。该条此前只存在于 automation memory 与 daily log，**本轮首次落进本手册**（见 §2.3 C14） | C14 |
| 30 | **把含转义序列的多行内容走 shell 参数通道**（heredoc / `python -c`）—— **带单引号的 heredoc 也不保反斜杠**，落盘后源码语法即坏 | 2 | 含反斜杠 / `\n` `\t` `\U` `\x` / 反引号的内容**一律 Write 成 `.py` 再执行**，写完先 `py_compile` | J4 |
| 31 | **单一权威文件被外部进程整文件覆盖**：本地 Edit 沉淀静默消失、无告警，长会话里只回验「我插入的串在不在」查不出来 | 1（但一条即可毁掉当日全部沉淀） | 每次 Edit 前后各取一次 `mtime + size` 指纹；**体量跃变本身就是最强告警**；收尾核对全文体量而非只查标记串 | W13 |
| 32 | **向「带编号体系的文档」追加逐字引用段落时，引用体把源文档的标题编号原样搬进来 → 制造同号不同义**（本轮 §7 抢救区一度同时存在两个 `### 3.2.5`；M7 的同族新形态：追加段落时写作惯性绕过体例约束） | 1 | 引用体一律**降级标题层级 + 加引用标记**（本轮：`### ` → `#### 【原文引用】`）；写入后**对全文做标题唯一性断言**（`###` 级重名数必须为 0），不能只看「我加的内容在不在」 | M7 新形态 |
| 33 | **把「沉默」读成「安全」** —— 检查不报 / 封锁不执行 / 依赖缺失使整节缺席，输出都与「无风险」一模一样 | 3+ | 三种形态：V3（永不 FAIL）· V4（永不执行）· **C11（压根没跑）**。共同防御：每条检查配 known-FAIL 样本并**实测它真的 FAIL** | V3/V4/C11 |
| 34 | **凭记忆手写替换/Edit 的匹配串**（缩进、行首形态）→ `count==0` 静默失效或整批 abort | 4 | 匹配串一律从当前文件 `Read` **原文抄录**；脚本先全量校验 `count==1`、全部通过后才写盘 | W3/W12 |
| 35 | **体例约束（禁 `**` 等）没进生成路径，只在写后验证层被拦** → 同一文档连续多轮重复引入，量级从 1 处涨到 48 处 | 2 | 体例写进模板/生成脚本头部（生成时即遵守）+ 写后门禁必跑且能 FAIL；一周内两次被同一体例拦下 → 动生成端，不加断言 | M7 |
| 36 | **把「一个产物目录」当成产能/进度的事实源** —— 目录与登记表不一致时没去查登记表，且把目录 mtime 的沉默读成「没产出」 | 1 次但连续 5 天未被发现，已污染简报、项目记忆与一条自律条款 | 状态类判断先定权威源（表.字段）再去查；**负向结论必须 ≥2 独立源交叉**；要写进记忆/规则的负向事实，落盘前当场测一遍 | V7 |

---
第 33–36 条为 2026-09-26 两条血脉（本地权威 / 运行时镜像）合并时追补，**尚未并入上方危害排序**；编号沿用增量追加以免打断既有「见 §1 第 N 条」交叉引用。
...
**J4 多行内容禁止走 shell 参数**（heredoc / `python -c` 字符串）。换行会变成字面量 `\n`。一律 Write 成脚本文件再执行。（**复发 2，形态见下**）

- **复发实例（2026-09-26，<user>e-model 每日 23:30 自动化）：单引号 heredoc 也不保反斜杠。** 用 `cat > scan.py << 'PYEOF' … PYEOF` 承载含双反斜杠的多行 Python 源码，落盘后 `dirpath.replace('\\','/')` 变成 `replace('\','/')` —— 反斜杠少一个，`SyntaxError: invalid syntax`，脚本一行未跑。**关键点：`<< 'PYEOF'` 的单引号本该禁用展开、保住反斜杠，实测仍被吞**，所以「带单引号的 heredoc 是安全的」这个假设在本环境不成立。
- **为什么会被误判为安全**：本条原文只写了「换行会变成字面量」这一种形态，容易被读成「不换行/加引号就没事」。根因是**内容经过 shell 参数通道时，转义序列不保证原样到达** —— 与 §2.3 C1b（反引号被命令替换后内容静默消失）、§2.1 J1（转义字面量破坏语法）同族；区别是本条**响亮失败**（SyntaxError 立刻暴露），C1b **静默失败**。
- **可执行判据**：凡要写的内容含任何转义序列（反斜杠 / `\n` / `\t` / `\U` / `\x`）或反引号，**一律用 Write 工具成 `.py` 再执行**，不走 heredoc、不走 `python -c`、不走双引号串；写完先 `python -m py_compile`（本形态下报错会在这一步就暴露）。
- **附带**：修正动作是**改写入方式**，不是改被测内容。当时若图快把源码里的双反斜杠改成单斜杠也能跑通，但那是「改数据去贴合坏闸门」的同族错误，下次同样的坑会换一个转义序列再犯。
...
**J8 消费结构化真源前，先打印一层 `keys()` 确认形状 —— 真函数的返回结构与"记忆/假设"不符时，先查契约，别在消费侧硬套**（2026-09-24 建，复发 1，§2.1 J1 表格末行的**镜像**形态）
...
**J9 同名后缀不代表同一种编码：读真源前先探首字符**（2026-09-24 建，复发 1）

- 现象：对 `.workbuddy/private/a-share-capital-plan.yaml` 直接 `json.load` → `JSONDecodeError: Expecting value: line 1 column 1`。而**同一目录**下的 `instrument-master.yaml` 用 `json.load` 完全正常。两个文件后缀相同、目录相同，编码却不同。
- 事实（本机 `.workbuddy/private/` 实测）：`instrument-master.yaml` = **JSON 编码的 YAML**（`json.load` 可读；**禁止** `yaml.safe_dump` 回写 —— 会把整文件从 JSON 形态换成 YAML 形态，契约被破坏且 diff 不可读）；`a-share-capital-plan.yaml` / `open_items.yaml` = **真 YAML**（`json.load` 直接炸，须 `yaml.safe_load`，而后者依赖 venv 里有没有 pyyaml，见 §2.3 C16）；`portfolio-state.json` = 真 JSON。
- 正确做法：① 读任何 `.yaml/.yml` 前**先探首字符** —— `open(p, encoding='utf-8').read(1) == '{'` 走 `json.load`，否则走 `yaml.safe_load`；② 该探测写成一个可复用 helper，**不在每个脚本里凭记忆选解析器**；③ `instrument-master.yaml` 的任何写回走文本级定向替换或 `json.dumps`，绝不 `yaml.safe_dump`。
- 判据：**文件扩展名是命名约定，不是编码声明。** 凭后缀选解析器，与 D1（凭标题推时点）、D9（凭记忆填运行值）是同一条错误的三个宿主。
...
- 判据：**保留** = 策划（PRD／策略／清单／变更日志／历史索引）+ 结果（最新交付版／报告／源码／云函数／配置／结果数据）；**清理** = 一次性脚本、导入中间数据、抓取页快照、控制台日志、旧版本副本、与在库文件 sha 完全相同的重复拷贝。<corp>传播类与含私有数据的项目不进自动范围，单列待确认。
...
- 防线② **skill／依赖副本先核验"是否已安装"再谈删**：`Claw/skills/` 下 `tushare-finance`、`youtube-watcher`、`humanizer-zh` 189 天未动，形态完全像废弃本地副本，实测**三个都未安装到 `~/.workbuddy/skills`**，是唯一副本 —— 按"长期未动=可删"处理就永久丢失。判据：`{p.name for p in (HOME/'.workbuddy/skills').iterdir()}` 比对，未命中一律保留。
...
**W13 单一权威文件可能被外部进程整文件覆盖，本地沉淀静默消失**（2026-09-26 实证，复发 1）

- **现象**：2026-09-26 23:31–23:36，`~/.workbuddy/ERROR-PLAYBOOK.md` 由 108,280 B（当日 08:43 长驻自动化写入后的状态）经我 4 次 Edit 扩到 **111,352 B**；**23:36:51 该文件被另一进程整文件覆盖为 179,683 B** 的仓库血脉版本（可定位来源：`github/error-playbook-private/ERROR-PLAYBOOK.md`，mtime 23:35；另有 `github/error-playbook/ERROR-PLAYBOOK.md` 180,676 B）。我的 4 处沉淀全部消失，**且消失时没有任何告警** —— 直到下一轮 grep 回验时 4 个断言串计数为 0 才暴露。
- **为什么比 §1 第 1 条（同文件并行 Edit）更危险**：那条至少发生在同一会话内、覆盖物是自己写的；这条是**跨进程、跨会话、事后无痕**，而且**恰恰回验不得**——「我插入的那串在不在」这种断言，在整文件被替换成另一血脉之后，会被其他内容干扰或直接判定为「我没写成」，把人引向「重写一遍」而不是「先查文件是不是被换了」。
- **可定位性来自体量**：108,280 → 111,352 → 179,683 这条体量轨迹本身就是最强的告警信号，**比任何断言都早**。任何单文件被 Edit 增量维护的收尾校验，都应把「全文体量 / 行数」纳入比对，而不只查标记串。
- **正确做法（四条）**：
  1. 每次 Edit **前后各取一次 `os.stat().st_mtime` + `st_size`** 作为指纹；**指纹变了而自己只改了 N 处** → 立即停在原地查覆盖，不要继续写；
  2. 长任务的关键文件，改完**立刻把全文副本落一份到不受同步影响的路径**（本轮抢救成功靠的是覆盖方自己留下的 `.trash/sync_20260926_mirror/_runtime/ERROR-PLAYBOOK.md.old`）；
  3. 发现覆盖后**先定位来源血脉，不要试图合并**——比对 mtime、体量、内容指纹，把被覆盖版本抢救成**显式命名的备份**（本轮 → `~/.workbuddy/ERROR-PLAYBOOK.runtime-lineage-bak-20260926.md`，111,352 B，md5 `8862eaa3dc`）；
  4. **两条血脉分叉本身作为待裁定上报**，由人决定以哪条为准、如何合并；agent 不得自行择优（本轮两血脉的 `§2.2 W 编号` 与 `§1` 条目编号**同号不同义**，合并会直接制造错误引用）。
- **判据**：**「文件内容是我写的」不等于「文件内容还是我写的」。** 凡存在「仓库镜像 ↔ 运行时副本」双向同步的文件，本地副本都不是唯一真源；写前先确认这一轮的方向是「仓库 → 本地」还是「本地 → 仓库」。

---

**W14 含中文的格式化串一律 f-string，禁用 `%-formatting`**（2026-09-21/22 连踩两次）。
两种形态，都是运行时 `ValueError`、都是写完才炸：
(a) 千分位：`'%12,.0f' % x` → `unsupported format character ',' (0x2c)`；
(b) **中文全角标点紧跟 `%`**：`'减 25%，第四理由…' % args` → `unsupported format character '?' (0xff0c)`。中文正文里出现「25%，」「占比%，」是常态，一写就中。
- 正解：一律 f-string 或 `.format()`；`%%` 转义只救得了想打字面 `%` 的场景，救不了全角标点。
- 自检：写完脚本先扫源码里的 `%` 占位符，只要同一个串里有中文标点，直接改 f-string。
**W15 中文串里的专有名词被截断，任何机器校验都抓不到**（2026-09-22 建，复发 4）。
- 现象：写作脚本的中文说明串里把 `noyb` 写成 `no`（原文「no 对文本的解读带立场」）。`py_compile` 通过、Notion 写入返回 200、读回核对标题与条数全部正确 —— **三处机器校验全部放行**，错误只存在于正文子块的一个词里，且不会导致任何后续失败。
- 根因：`py_compile` 只做语法解析；读回核对按 §3.1 第 5 条只核对标题与字段值。**专有名词的完整性不属于任何一条现有断言的判据**。§3.1 第 1 条要求「回验关键标记串」，但标记串是**我预期改动的地方**，专有名词不在这个集合里 —— 于是它成了验证的盲区。
- 正确做法（三选一，成本从高到低）：
  1. 脚本跑前对每条中文串做**专有名词白名单扫描**：把本轮出现的机构名 / 产品名 / 人名缩写列成清单，逐串 `in` 检查，缺一即报；
  2. 等效且更省事：**跑前把候选列表整体 Read 一遍**（不是只扫语法），专有名词截断与句内自相矛盾都在这一步暴露 —— 本轮就是靠这一步发现的；
  3. 禁止在中文说明串里使用会被自然语言吞掉的短缩写（`no`、`is`、`a`、`in`），一律写全称。
- 判据：**机器校验的通过只能证明「结构合法」，不能证明「正文正确」。** 凡正文串含专有名词，跑前必须有一次面向人的通读。
- 同族关系：与 §1 榜第 23 条（声明了某效果但执行层不读）同源 —— 都是「校验通过」与「内容正确」被混为一谈。区别是那条错在机器，这条错在**校验的覆盖面**。
- **复发实例（2026-09-25，形态是「防护措施自己失效」）**：给当日 daily-scout 脚本加专有名词白名单（本条「正确做法 1」）时，白名单里写了英文形式 `ByteDance`、`GLM`，而正文串用的是「字节跳动」、且全程从未提及 GLM —— 检查报 `FAIL 4 whitelist missing`，指向的是**白名单写错**，不是正文缺字。
  - **判据：白名单的词形必须取正文的实际语言形式**（中文正文写「字节跳动」，白名单就写「字节跳动」；中英混排以正文出现的形式为准）。词形不一致的白名单**恒 FAIL**，而恒 FAIL 的检查与 V3（恒 PASS 的装饰）同样是坏判据 —— 更危险的是它会训练出「FAIL 就改白名单」的习惯：等真有一次专有名词被吞掉时，同一种 FAIL 会被当成老毛病放过去。
  - 顺序：先按 §3.2.1 问「这个检查有没有能力 FAIL」，再问「它 FAIL 的时候指向的是不是真问题」。

- **复发实例（2026-09-26，daily-scout；形态与 9/25 完全同根，第三次由我自己复现）**：白名单首轮 FAIL 报 `['Trillium', 'Industry AI Foundry', '外部空间']`。三个里两个仍是**白名单自己写错** —— `Trillium` 从未进入任何正文（多写了一个词），`Industry AI Foundry` 正文实际是中文词形「行业 AI Foundry」。**根因与 9/25 一字不差：判据串按「我记得正文写了什么」写，不是从产物当前文本里取的。** 同日另有一条同族（§3.2.6 的 select 选项门限严于书面规则），已合并沉淀到 §3.2.6。
  - **本轮新增的可执行判据**：白名单/断言串**不止要词形对齐，还要「只收录正文真的用到的词」**。多写一个正文没有的词 → 恒 FAIL，与写错词形同害。收尾时应把白名单分成「应存在」与「不应存在」两栏，后者用 `not in` 判，而不是混在一处等它 FAIL 了再去分辨。
  - **复发的真正教训**：这条规则 9/22 建、9/25 强化、9/26 再犯，三次都发生在**同一个动作**（给自己新写的校验脚本填白名单/断言串）上。说明「Read 一遍再抄」这个动作没有被固化成流程 —— 下次写白名单前，第一件事是先把候选词在产物文本里 `grep` 一遍，命中的才进白名单。

- **复发实例（2026-09-25 当日第二次，<user>e-model 每日 23:30 自动化；形态是「判据词形与产物文本不一致」，即本条根因第一次由我亲手复现）**：给 CORE.md 新增的 2026-09-25 条目写写后断言时，断言串写成 `'主表 153 行'`、`'已发布 ≤ 9/25 141 行'`（纯文本形态），而落盘正文是 `主表 **153 行**`、`已发布 ≤ 9/25 **141 行**`（含 markdown 加粗标记）→ 断言在**完全正确的文件上**报 FAIL，rc=1。
  - **与上一条实例同根不同宿主**：上一条是「白名单词形取了英文，正文是中文」，这条是「断言词形取了纯文本，正文带 markdown 标记」。共同根因 = **判据串是按「我记得正文是什么样」写的，不是从产物当前文本里取的**。凡断言串涉及标点、加粗、空白、全半角、中英混排，一律视为**待验证词形**，先 `Read` 产物该行再抄。
  - **额外诊断线索**：本次 FAIL 只列出 2 个缺失串、其余 3 个命中 —— **同一条判据里部分命中、部分不命中，且不命中的都含特殊标记**，是词形问题的典型指纹（若产物真缺内容，通常整段判据同时失效）。
  - 顺序不变：**FAIL 先按脚本 bug 查**（§4 第 2 条）—— 本次正确动作是改断言，不是改文件；若当时图省事去改 CORE.md 把 `**` 去掉，就会为一个坏判据破坏另一份文档的既定体例。

- **正面实例（2026-09-26，<user>e-model 每日 23:30）**：给 `PROFILE.json` 的写后校验填 5 个断言串时，**先逐条核对产物实际文本**，发现原打算写的 `2026-09-26 奥森` 在产物里根本不存在（正文写的是「奥森 30.38km / 2:42:31」），当场把断言改成 `奥森 30.38km` 后才运行 —— **零成本规避一次假 FAIL**。这是本条「正确做法 2：跑前把候选列表整体 Read 一遍」的可执行形态，也是 9/26 新增判据「白名单/断言串只收录正文真的用到的词」的第一次成功应用：**填断言串之前，先在产物里 grep 一遍，命中的才进断言。**
**W16 画像/台账类文件的头部「上次更新」必须等于该文件内的最大日期**（2026-09-23 建，复发 2；与 2026-09-09 发现的 CORE.md 双日期漂移同族）。

- 现象：`THINKING.md` / `VOICE.md` 头部「上次更新：2026-09-06」，而两文件正文已被后续多轮自动化推进到 09-21（THINKING 新增交互规则节）/ 09-22（VOICE 校准表最后一行）——**漂移 15–16 天，连续 6 轮自动化都声称"已更新 THINKING.md / VOICE.md"，却没有一轮动过那行头部**。更早的同类事故：CORE.md 头部写 09-06、系统元信息段停在 08-18，漂移近三周（2026-09-09 核实发现）。
- 根因：头部日期**没有任何断言覆盖**。每轮自动化只校验"我插入的那一行在不在"（§3.1 第 1 条问的是唯一标记串），头部既不是标记串也不参与任何比对，于是它对 6 轮写入完全免疫。与 V4（声明 ≠ 执行）同族：文件头部的自述是散文，不是被机器守住的字段。
- 正确做法（可执行）：① 每轮更新画像类文件后，**先算「该文件的结构化更新记录」里的最大日期**（THINKING.md 取 `上次蒸馏：YYYY-MM-DD` 的最大值；VOICE.md 取第八节校准记录表末行首列日期），与头部日期比对，头部不得早于它，否则改头部；② 头部行写成可 grep 的唯一形态（`> 上次更新：YYYY-MM-DD`），纳入校验脚本断言；③ 已被多轮"更新"却头部仍停在旧值 → 优先怀疑头部从未被纳入写路径，而不是内容没写进去。
- **判据本身也要过 known-good 样本（本条第一版就错在这里，2026-09-23 当场修正）**：我第一版写的判据是「头部日期 == 文件内最大日期」，**在完全正确的文件上恒 FAIL** —— THINKING.md 正文含未来赛事日期 `2026-10-11`（海淀全马），文件内最大日期天然大于头部；VOICE.md 头部修复后含 `2026-09-23` 亦然。这是 **V3 的镜像形态**：V3 是"永远 PASS 的装饰"，这条是"永远 FAIL 的噪声"，**两者同样是坏判据**，区别只是坏得一目了然。正解 = 把比较对象限缩到**有界且语义明确的集合**（"更新记录段"），而不是全文所有日期。
- 判据：**头部日期是文件对外的自述，任何"我更新了这个文件"的动作都必须同时更新它**；改完查的是「头部 vs 更新记录段」，而不是「头部 vs 全文所有日期」、也不是「我插入的内容在不在」。
**W17 替换/Edit 的匹配串必须从当前文件原文抄录，不能凭记忆手写（2026-09-24 建，复发 4；W3 的批量变体）。**

- 现象：压缩 `MEMORY.md` 的脚本里，`old` 串写成 `- 主语硬筛：…`，文件实际是 `5. 主语硬筛：…`（编号规则，不是 bullet）。首条 `count=0` 命中即 abort，整批未执行。**若是「边替换边写盘」的写法，就会静默漏改这一条而其余全部落盘**——9/24 的脚本因为设计成「全部校验通过后才一次性写盘」，才把损失限制在「没跑」而不是「跑坏」。
- 根因：`old` 串是**按印象手写的**，而印象里的行首形态来自更早的版本、另一份文件或另一种列表风格（bullet / 编号 / 缩进）。同一文件被多轮脚本改动后，**行首形态是最先漂移、最难凭记忆命中的部分**，与 W3（缩进失配）同族。
- 正确做法：① 每个 `old` 串先 `Read` 该行**原文**再抄进脚本，不凭记忆重建；② 脚本结构固定为「先全部校验 `count==1` → 全部通过 → 再一次性写盘」，禁止边替换边写；③ 任一条 `count!=1` 即 `sys.exit(1)` 并打印该条前 60-70 字，便于定位；④ 一条 abort 后**先查脚本里的 `old` 串**，不是先怀疑文件、更不是重试同一条命令（§4 归因纪律）。
- 判据：**「凭记忆写的匹配串」与「从文件读出的匹配串」是两种状态，前者一律按待验证处理，不按已知处理。**

- **复发实例（2026-09-24 当日第二次，<user>e-model 23:30 自动化，即本条建档同日）**：给 `VOICE.md` 第八节校准表追加行时，`old_string` 凭印象重写整行——漏掉「标的与流程名（meta / ptl）一律小写裸写」「价格数字不带货币符号」等原文片段，Edit 直接报 `String to replace not found`。
  - **形态与首例相反、根因相同**：首例是**行首形态漂移**（记忆里是 bullet，文件里是编号 `5.`），本例是**长行中段被凭印象压缩**（把 300 字的行按大意重写）。两者都是「匹配串的来源是印象，不是文件」。
- **复发实例（2026-09-25，`ip-brand/.workbuddy/memory/MEMORY.md` 压缩脚本）**：需要改的一行原文是「…目标 60/20/20 → 人格层缺约 18 篇。**每月固定 2 篇单独排期。**清单 …」，我凭印象把中间那句跳过去、写成「…18 篇。清单」，首条 `count=0` 即 abort。**这是本条规则第 3 次在同一个文件、同一种压缩脚本上复发。** 两条可用结论：
  - ① 「全部校验通过才写盘」的结构确实把损失限制在「没跑」，但**它不降低复发率**，只降低单次危害 —— 不要把「没写坏」当成「已经改好了」；
  - ② 同一文件被多轮压缩后，**最先漂移的不是行首，是我对「这一行大概长这样」的整体印象**。压缩轮次越多的文件，越不能靠印象定位，`Read` 的成本相对于「重新定位一遍」是零。
  - **本例未造成数据损失，原因值得记下**：Edit 的失败是**响亮的**（count==0 立刻报错、文件未被触碰）。若换成脚本里「边替换边写盘」的写法，同一段会**静默漏改而其余全部落盘**——这正是首例已经写下的结论，本例把它反向验证了一遍。
  - **防御升级（可执行）**：凡匹配串长度超过约 80 字，或含括号/编号/中英混排，**一律先 `Read` 取该行原文**（脚本里就 `print(repr(line))`）再整行粘贴，禁止在脑内重排句子。行首形态（bullet / 编号 / 缩进）每次都必须从原文取，不得沿用上一轮记忆。
**W18 用 `python -c` 承载含反引号的内容 = 反引号被 shell 当命令替换，内容静默消失（2026-09-25 建，复发 1；§2.1 J4 / §2.3 C1 的变体）**

- 现象：改本手册时，用 `python -c "…"` 走 **bash 双引号串内联**了一段中文 + 反引号包裹的标识（`open_items.yaml` / `ON_TRACK` / `gate.py` 等）。脚本 `rc=0`、写盘成功、行数与字节数都正常，**回验的「关键标记串存在」也全部 PASS**（因为标记串本身不含反引号）。实际落盘文本里所有反引号包裹的片段被替换成空串，句子读作「…： 写 ，而  一过， 算出的 verdict 即为 。…」。
- 根因：**bash 双引号内反引号仍执行命令替换**，替换目标恰是这些看似"文件名"的词；被替换后 bash 把 `command not found` 打到 **stderr**，而脚本自身的 stdout 与退出码完全正常。这是 §3.0 假成功清单里的第 8 类：**回执 / rc / 预期标记串三重校验全部通过，错在"内容被 shell 改写"**。
- 为什么回验没抓到：标记串回验查的是「我预期的唯一串在不在」，而被吞掉的恰是**我没预期会变的那些词**。与 W10（专有名词截断）同一盲区 —— 机器校验只能证明结构合法。
- 正确做法（可执行）：① **写文件内容一律走 Write / Edit 工具，或 Write 成 .py 脚本再执行**（本就写在 §2.3 C1 与 §2.1 J4，本次是我自己违反）；② 若确实要内联 Python，用**单引号 heredoc（`<<'EOF'`）**而非双引号串 —— 单引号 heredoc 内反引号不替换；③ 写完必须**再 Read 一遍插入段并肉眼通读**（本次是靠通读同一段才发现，标记串回验当时全绿）。
- 判据：**「我预期会变的地方」不等于「会被改坏的地方」。** 检测手段必须覆盖整个插入块，不能只查几个预期标记。
...
**F4c 翻转一条规则 = 全文搜同义表述，不是改一处就完**。改掉一条硬规则（如"新规则需人类批准"→"agent 当场写入"）时，同一主张会以不同措辞散落在多个位置：操作步骤、Human-in-the-loop 清单、Failure Handling 表行、输出模板的一行、`references/*.md`、README / README_zh。**只改主条目会留下 5-6 处与新规则直接矛盾的文字**，而自相矛盾比规则本身更容易被扫描器判为问题（会反向升级）。→ 改完主条目后，用规则里的**关键词**（approval / propose / draft / 人工确认 / 提议）全仓 grep 一遍再收工。实证：2026-09-20 workflow-guard-rails 1.0.4 只翻了 Hard Rule #7，遗留 6 处矛盾表述，1.0.5 才补齐。
...
- **2026-09-26 附（coreutils 存在时仍未先探）**：本会话 `mkdir` / `cat` / `ls` / `head` 实测可用，但**使用前没有按本条探一次**，直接写了 `git log | head -8`。结果正确、无损失，仍记为偏离：探一次的成本是一次调用，省掉的是环境变化之后那次静默失败。
...
- **【复发 1 · 2026-09-21 全仓复扫】** 同一坑再次出现在三个不同项目目录：`Claw/data-ai-daily-brief/覆盖`、`Claw/{invassistant,research,data-ai-daily-brief}/nul`（`nul` 是 Windows 保留设备名，普通 API 删不掉，**必须走 `\\?\` 前缀路径**）。上次登记的三例（`建立`、`触发：2026-09-21`、`边界：…`）已不在盘上 —— 说明**只清了产物没断生成路径**，C1b 会持续复发。
  - **新增判据**：这类文件的签名是「**0 字节 + 文件名是句子/保留名**」。扫 `~/.workbuddy` 级目录时用 `st_size == 0` 全量筛，再人工看文件名是否像文件。
  - **【2026-09-21 判据修正：禁止只按 0 字节删】** 实测工作区全仓 0 字节文件 **397 个**，其中绝大多数是 `Claw/invassistant/.venv/Lib/site-packages/` 下**合法的空 `__init__.py` / `py.typed` / `*.pyi`**，只有 4 个是真垃圾（`覆盖` + `nul`×3）。**"0 字节"单独不构成删除依据**，必须叠加：① 文件名是句子/中文短语/保留名；② 排除 `.venv`/`site-packages`/`node_modules`/`__pycache__`/`.git`；③ 逐条人工过名。**批量按 0 字节删 = 直接毁掉 Python 环境。**
...
- 2026-09-21 新形态（复发 5+）：**环境变量本身为空**——新 shell 里 `$TMPDIR` 未导出，`curl -o "$TMPDIR/x.json"` 实际写到根目录 `/x.json`，exit 0、`wc -c` 有数、全程无报错（假成功），下一步按 Temp 路径找文件才 FileNotFoundError。防御：临时文件不依赖环境变量，直接写绝对路径（如 `/c/Users/<user>/AppData/Local/Temp/`）；落盘后用同一路径 `ls -la` 回验。
...
- 本机三个 `python` 各不相同，且**其中最像对的那个是错的**：
  ...
  | `~/.workbuddy/binaries/python/envs/default/Scripts/python.exe` | ✅ | **正确入口**（`envs/invassistant` 亦可） |
...
- 正确写法（**绝对路径**，两个 env 都带 pyyaml）：
  ```
  PY="$HOME/.workbuddy/binaries/python/envs/default/Scripts/python.exe"   # 或 envs/invassistant
  "$PY" scripts/gate.py
  ```
...
**C16 托管 venv 缺依赖 → 检查静默降级为「0 命中」（2026-09-21 建，复发 1）**
- 现象：`scripts/gate.py` 第 2 步（治理回归）与第 3 步（台账到期）同时 FAIL；第 3 步自报「需要 pyyaml：请用 envs/default/Scripts/python.exe 运行」，而实际**正**是用该解释器跑的。第 1 步派生价位校验仍 PASS，但输出里「待裁定=0」。
- 根因：`C:\Users\<user>\.workbuddy\binaries\python\envs\default` 没装 pyyaml。`validate_levels.py` / `check_open_items.py` 在 `try: import yaml` 失败时打印提示后**继续往下走**，C9 一节整个缺席 —— **待裁定项由 1 条静默变成 0 条**。
- 危害：与 V3（检查永不 FAIL）、V4（封锁永不执行）同族，是第三种形态 —— **检查压根没跑，输出形态与「无风险」完全一致**。而「待裁定=0」会被下游读成「本轮无未闭合项」。
- 防御：① 门禁脚本**不得**用 `try/except ImportError + print` 降级，缺依赖必须 `sys.exit(非 0)`（fail-closed）；② 「命中 0 条」这类输出必须附「本轮实际扫描了 N 条」的计数，0/0 与 0/3 必须可分辨；③ 换机器 / 换 venv 后第一次跑门禁，先 `python -c "import yaml"` 探依赖。
- 实测：装 pyyaml 6.0.3 后重跑 gate → 4/4 PASS rc=0，C9 恢复命中 OI-004。
**C17 Glob 默认 limit=100 会静默截断"跨工作空间全量枚举"（2026-09-22 建，复发 1；与 §2.7 D7 变体同族）**
- 现象：<user>e-model 每日 23:30 自动化用 `Glob **/.workbuddy/memory/*.md`（path 指向 `C:\Users\<user>\WorkBuddy`）枚举记忆文件，返回 100 条即止 —— 本机实际有 **522 个**记忆 md。返回的 100 条按 mtime 升序排列（头部是 2026-03 的文件），**当日与前一日的文件全部落在截断之外**。若直接把这 100 条当"全部记忆文件"用，当日两份有效日志（ip-brand 08:35、invassistant-private 22:36）会被判为"不存在"→ 本轮更新直接归零。
- 根因：**工具默认 limit 参数 + 排序方向**，两者都不会报错或提示截断；与 C1（管道静默丢 stdout）同属"输出形态与真实情况不一致"，与 D7 变体（清单抄上一轮结论）同属"漏文件"族。
- 正确做法：① 跨工作空间枚举**不依赖 Glob 的默认值** —— 显式 `limit` 放大到远超预期数量，或直接用 Python `os.walk` 递归（可顺带剪枝 `_CLEANUP_BACKUP` / `node_modules` / `\.git\`）；② 枚举后**打印命中总数**做 sanity check（本轮 522），总数与上一轮同量级才说明扫描完整；③ 排序方向不要假设，按需自行 `sort`。
- 判据：**枚举类操作必须有一个"应扫到多少"的独立计数** —— 只看到"我列出了 N 条"而不看"总共有多少条"，等于没有做完整性校验。
**C18 在 Python 里 `subprocess` 调 `bash` 会落到 WSL 入口并失败（2026-09-22 建，复发 1）**
- 现象：校验脚本内 `subprocess.run(['bash','-lc',"printf 'a\nb\nc\n' | head -2"])` 取回的 stdout 不是命令输出，而是**一坨二进制垃圾**：UTF-16 乱码夹杂 `wsl: 检测到 localhost 代理配置，但未镜像到 WSL` 与 `Bash/Service/CreateInstance/MountDisk/HCS/ERROR_PATH_NOT_FOUND`。完全不报 Python 异常，`returncode` 也正常。
- 根因（推断）：本机 `bash` 在**子进程环境**里解析到 WSL 入口（Bash 工具另有自己的 shell 初始化和工作目录映射），WSL 实例起不来就返回错误块。
- 防御：写 `.py` 校验脚本时**不要 shell out** —— 路径 / 文件 / 哈希 / 计数全用 Python 原生（`os` / `io` / `hashlib` / `shutil`）实现；确实需要跑 shell 的，**用 Bash 工具直接跑**，不在 .py 里 `subprocess('bash')`。
- 判据：**"输出"先看它是不是可读文本** —— 二进制乱码一律按失败处理，不看 exit code、不看有无异常。
- 同族：与 §3.0 六类假成功同类（回执形态与真实结果无关），只是这一类的回执是"垃圾字节"而不是"成功"。
**C19 工作区根目录存在 `nul` 文件时，ripgrep 在根目录下报 exit 2 并返回零结果（2026-09-24 建，复发 1）**
- 现象：`Grep` 工具的 `path` 指向**工作区根目录**时，报 `ripgrep exited with code 2: rg: <workspace>\nul: 函数不正确。 (os error 1)`，整次检索**一条结果都不返回**。若把它当成「没有命中」，就会误判「内部查重干净」。（本工作区根目录确实存在一个名为 `nul` 的文件。）
- 根因：`nul` 是 Windows 保留设备名，在 Git Bash 里一次 `> nul` 重定向或某个脚本写出就会落成同名普通文件。ripgrep 遍历到它时无法 `stat`，**以 exit 2 终止整次遍历** —— 不是跳过该文件继续，而是整个搜索失败。
- 危险形态：与 C1（管道静默丢弃 stdout）同族 —— 报错文本短、容易淹在长输出里，而「零命中」与「没写过」的输出形态完全一致。落在内部查重环节（防线二）时会把 A 态直接判成 C 态，且不产生任何后续失败。
- 正确做法：
  1. **`Grep` 的 `path` 不指向工作区根**，逐个指向子目录（`content/`、`_topics/`、`_strategy/`、`_reference/`、`_operations/`）。本工作区实测这五个子目录全部正常，根目录必失败。
  2. 看到 `exit code 2` / `os error 1` 一律按**检索失败**处理，**不得按「无命中」处理**，换 path 重跑。
  3. 根目录的 `nul` 应清除：`[System.IO.File]::Delete('\\?\<abs path>\nul')`（Windows 保留名须 `\\?\` 前缀）。已删除则本条目降为历史记录；在确认删除成功之前，一律按第 1 条执行。
- 判据：**「零命中」与「检索失败」必须是两种不同的输出形态。** 工具返回非零退出码时，该次检索的结果不可用于任何结论。
**C20 当日记忆的落点不止 `memory/YYYY-MM-DD.md` 一种形态；只按文件名找当天日志会整日漏读（2026-09-24 建，复发 1）**
- 现象：<user>e-model 每日 23:30 自动化的流程描述是「对每个 .workbuddy 目录，检查 `memory/YYYY-MM-DD.md`（当天日期）」。2026-09-24 实测：`Claw/ip-brand` 工作空间当天**没有** `2026-09-24.md`，当日全部产出写在另两处 —— `memory/MEMORY.md`（长期记忆，当日增量改写，14,511B）与 `automations/automation-1784357183801/memory.md`（自动化运行记忆，49,260B，末尾新增「2026-09-24 执行摘要」整段）。若严格按文件名规则枚举，该工作空间当天的 daily-scout 全部内容（0 P0 / 1 P2 / 5 P3、防线三拦下四个真候选、R2 底稿事实修正、结构性请示）会被判为「当天无活动」，**而漏读的输出形态与「确实没活动」一模一样**。
- 根因：**「当天有活动」这个事实没有单一的文件名承载**。至少四种落点：①`YYYY-MM-DD.md` 日期日志；②`MEMORY.md` 长期文件的当日增量段；③`automations/<id>/memory.md` 自动化运行记忆的当日段；④工作空间根目录的当日产出文件（报告 / 简报 / 底稿），它们不在 `.workbuddy/memory/` 下。按文件名找，只能覆盖第 ① 种。
- 正确做法：① **枚举按「mtime 落在当日或前一日」过滤，不按文件名匹配** —— 对 `.workbuddy/memory/` 全树（含 `automations/` 子目录）所有文件取 `os.stat().st_mtime`，当日/前日命中即入清单；② 清单打印**命中总数**并与上一轮同量级比对（本轮 535 vs 上轮 525）；③ 对每个工作空间再做一次「当天是否有产出」的旁证检查（工作空间根目录当日 mtime 的新文件），避免全落点都在 `.workbuddy/` 之外；④ 产出「当天无活动」结论时，必须能说清**扫了哪些落点、各扫到几条**，而不是只说「没找到今天的日志」。
- 判据：**「当天没活动」与「我没扫到」必须是两种不同的输出形态。** 与 C11（venv 缺依赖使整节检查缺席）、§1 第 25 条（把沉默读成安全）同族 —— 都是「缺席」被误读为「无事」，区别只是缺席的单位从一节检查变成了一个工作空间的整天内容。
...
**N1 归因纪律**：一次失败禁止下"网络层被拦截""TLS 被墙""权限不足"的重结论。流程：① 同一命令重试 1-2 次；② 换栈交叉验证（curl / Node https.request / Python urllib 三者轮换）；③ 仍失败才上报，且必须附重试次数 + 交叉验证结果。禁止一次失败就给科里甩 A/B/C 三条路。
- 实证：curl 测 4 个域名，Notion 两个返回 000 + schannel 握手失败，立刻判定"公司电脑针对性拦截"——10:17 被科里截图打脸，重试一次立刻 200。

**N2 公司代理误报 5xx**：urllib/WebFetch 对「301 → DNS 未配置域名」返回 **502**（真实是 301）；`dns.google` DoH 也 502。→ urllib/WebFetch 报 502/503 **先用 curl 交叉验证**，再 `curl --noproxy '*'` 三重确认。
...
**N3b 子代理 429**（使用量超频率限制）不重试子代理，直接降级为自己 WebSearch（独立配额）。
...
**N9 tencentdocs CLI 全端点 502「Tunnel connection failed」但 curl 同端点 200**：根因不是网络/代理故障，是 V2 网关凭据 provider 下发的 `apiBase`（实测 `https://www-docs.workbuddy.cn`）覆盖官方域，公司代理隧道拒绝该域 CONNECT。诊断路径：① `TDOC_DEBUG=1` 重跑，看 `api_base=` 实际解析到哪；② curl 同端点交叉验证（N2 纪律）区分"隧道真挂"与"栈差异"；③ 同刻 urllib 裸测对照。修复（已固化在项目 `_tools/_td_retry.py`）：进程内 `os.environ["TDOC_API_BASE_URL"]="https://docs.qq.com"` 钉死官方域（env 优先于 provider 覆盖，代码里 `if not api_base` 分支）+ 失败重试 8 次×3s。**禁 scrub 代理走直连**：公司网 DNS 离代理不可解析（getaddrinfo failed），127.0.0.1:52644 隧道是必选。复发 1。
...
- **<site-domain> 站点源仓库不在本工作区**：`C:\Users\<user>\WorkBuddy\<site-domain>`（remote = `github.com/<github-user>/<site-domain>`，GitHub Pages + CNAME 服务 <site-domain>）。论文篇合集 = `/papers/`，欧洲系列 = `/europe/`。本工作区 `cloudbase/hosting/` 下的 paper-map/personal 只是历史镜像副本。**给 <site-domain> 改页面前先对该仓库 fetch 对账**——其本地 checkout 曾落后远端（2026-08-28 远端历史被重写 force-push，本地还是旧链），直接 commit 必然 non-fast-forward。
...
**G10 副本一致性对比必须取四层状态，只看文件内容会把「未提交改动 + 历史分叉」读成「当前状态」**（2026-09-27 实证，复发 1）。
- **实证（ERROR-PLAYBOOK 镜像对比）**：对比报告用 `open().read()` 取三方文件内容算 md5，得出「公开镜像 180,682 B / md5 `a03d3b8a`」。补跑 `git status` 才发现三件事全被漏掉：① 那个 md5 是**工作树（未提交）**的内容，仓库 HEAD 是 180,676 B / `b67b89b0`，两者不是一回事；② 两个镜像仓都是 **`ahead 1 / behind 2`** —— 与远端历史**分叉**（远端两个 commit 是 `Initial sanitized public mirror` + `Add README`，本地链 `4cc132a → b5e9843 → 7ef9668`，两边无共同祖先）；③ 本地缺远端独有文件 `README.md`。这三点在「只比文件内容」的做法里**完全不可见**，而它们直接决定「能不能推、怎么推」。
- **根因**：把「读到的文件内容」等同于「版本库状态」。文件系统只有一个维度，git 有三个（工作树 / index / HEAD），加远端是四层。
- **正确做法（四层取材，缺一层则「一致」的结论不成立）**：
  ```
  ① 文件系统    md5 + size + mtime            当前内容
  ② git 工作树  git status --short            未提交改动（常是最新编辑所在）
  ③ git HEAD    git show HEAD:<file> | md5    已固化内容
  ④ 远端        git status -sb 的 ahead/behind 分叉状态
  ```
- **工作树常常比 HEAD 新**：本轮镜像工作树里有未提交的补完改动（旧 skill 名 `workflow-guardian` → `workflow-guard-rails` 的收尾清理），报告只引 HEAD 会漏掉它，只引工作树又会把它当「已发布状态」。**两个数都对，但必须分别标明是哪一层。**
...
**P6 shim 下 npm 全局 CLI 包装脚本坏**（依赖 sed/dirname → MODULE_NOT_FOUND）→ node 直调 `C:/Users/<user>/AppData/Roaming/npm/node_modules/clawhub/dist/cli.js`。skillhub 同理绕过 wrapper 直调 `skills_store_cli.py`。
...
**P10 脱敏**：`<site-domain>` 系真实域名、真实 GitHub 用户名、"公司代理环境"表述 —— 私有库可留，**公开发布前必须替换成 example.com 占位符并去除公司环境表述**。发布前三件套：claim-to-source → cross-material-consistency → localization；P0 未清零不发布。
...
**P14 skillhub 没有可读接口，发布状态无法远程核实**。`GET /api/v1/community/skills/{slug}`、`.../mine`、`.../list`、`.../rankings` **全部 405**（带 Bearer 也一样），只有 POST publish 通。→ 判断"某 skill 是否上过 skillhub"改用旁证：clawhub `inspect <slug> --versions` 有 @<github-user> 记录 + GitHub `<github-user>/<slug>` 存在（三平台同步惯例）。**不要用 DELETE 探活（P7），也不要为了确认就先发一版（发布是对外动作，需确认门禁）。**
...
**P18 同步 / 发布必须是批次里最后一个动作，排在上游仍会变更时 = 同步完成即滞后**（2026-09-27 实证，复发 1）。
- **实证（ERROR-PLAYBOOK 镜像同步）**：把镜像从 canonical 重建并 commit 完成后（公开 md5 `7ca123eb` / 私有 `50cba495`），紧接着往 canonical 追加了本轮沉淀的三条（`§3.2.8` / `§3.2.9` / `§2.5 G10`，+6,280 B）→ **刚同步完的镜像立刻重新滞后**，同步动作白做一遍，还得再跑一次。
- **根因**：批次里有依赖顺序的动作被排成了「先同步、后继续改上游」。同步是**终端动作**，它的输入（上游文件）必须在它之前冻结。
- **正确做法**：
  1. 开工前把批次动作分成「产出层」（改上游）与「同步层」（推下游），**同步层只放最后一个**。
  2. 若同步之后又改了上游，**必须整份重跑同步**，不能只补差异 —— 补差异会造出第二个漂移点，且下次核对时无人知道差异边界在哪。
  3. 报告里必须写明**同步快照对应上游的哪个版本**（md5 + 体量），否则下次无从判断镜像对的是哪一版。
- **判据**：动手同步前问一句「我接下来还会改上游吗」。答案是会 → 现在就别同步。
- **与 P17 的区别**：P17 是「一次推送要覆盖多个面」（横向漏面）；本条是「一个面要等上游定稿」（纵向时序）。两条同源于一类误判 —— **把「动作执行过」当成「状态已一致」**。
...
**D16 登记型派生值必须能由其输入重算；滚动窗口类参数必须带窗口边界**（2026-09-22 建，复发 1）

- 现象（两类，同一次运行内命中同一个真源文件的同一区块）：
  (a) `instrument-master.yaml → range_position_rule_v223.profiles` 的 `percentile_52w` 与其**自身**的 `range_low` / `range_high` / `last_price` 不自洽 —— 8 项中 6 项偏离（META 53.7 vs 自算 59.9｜AXP 38.3 vs 21.0｜AAPL 72.9 vs 93.3｜COST 29.6 vs 19.7｜MCO 52.4 vs 41.3｜GOOGL 53.6 vs 64.4）。用区块内任何两个字段反算就能立刻发现，但没有任何闸门做这件事。
  (b) META 区间高登记为 791.00（自述「同 52 周高」），实测它是 **2025-09-19 的盘中高 790.80**；至 2026-09-22 该高点已滚出 12M / 250 交易日窗口，真实区间高为 **770.60**（全历史最高 796.25 @2025-08-15）。连带口径后果：触发带下沿 711.90 → 693.54、反触发线 814.73 → 793.72。
- 根因：**派生字段（分位 / 区间高 / 20D 高）是唯一会被时间自然改写的一类运行值** —— 没有事件触发它们更新，只有日历能改；而「下次月度复核」是声明，不是机器行为。与 **V4**（声明 ≠ 执行）同族，与 **D1**（时点靠推断）同源。
- 隐蔽性：两处都不产生 FAIL、也不改变当日结论（判定依赖距带% 与 MA200 门槛），因此**不会被任何现有检查拦下**，只会在下一个引用它的人手里变成错判。
- 正确做法：
  1. 登记派生值时**同批写「窗口边界」**（例 `range_high_window: [2025-09-23, 2026-09-22]` + 取样日），使「是否已滚出窗口」可机械判定；
  2. 校验脚本对每个派生字段做一次**由其输入重算**的对拍（分位 ← hi/lo/last_price；区间高 ← 带窗口的 K 线切片），偏差超阈值即 WARN。**不能重算的派生值等于无法验证的值**；
  3. 脚本读区间 / 分位类字段时，**优先用当次实时重算值，登记值仅作对照**，并在报告里并列两侧。
- 附：**补偿常量必须自带失效条件。** 当真源汇总字段滞后于一条已登记事实时（本轮：`accounts.US.cash` 未含 META 减仓 +7,120.00，而 `holdings[META].shares` 已改为 30），在脚本里显式加一个补偿常量是可行的过渡做法，但**必须在同一处写明「真源刷新后删除此常量」** —— 否则刷新落地那天会静默双重计数。

---
**D1b 日戳脚本「机械替换日期」不会修语义标签（2026-09-23 建，复发 4）**
- 现象：持仓检查自动化每日用 `s.replace('20260922','20260923')` 生成当日脚本，脚本尾部有一行 `print('真源校验：… 新浪 prev_close（= 2026-09-21 收盘，晚一个交易日）')`。该标签在 9/22 版**正确**，跨日后即失效 —— 9/23 时 `prev_close` 已是 9/22 收盘、落后 2 个交易日，而机械替换只动日期字面量，不动「晚一个交易日」这个相对措辞与括注的绝对日期。
- 根因：**日期戳只保证「文件名的日期」被改对，不保证「文件内容里任何关于时点的断言」被改对**。与之同源的措辞还有「昨日 / 上一交易日 / 与昨日 delta / 距上一次执行 N 日」——全是相对或自指的时点表述。
- 正确做法：① 复用日戳脚本时，先 grep 全部时点措辞（`上一交易日` / `昨日` / `收盘` / `晚一个` / `距今`），逐个按当日实际改写；② 更优：让脚本**自己算**（`prev_close` 的时点 = 上一交易日，可由 K 线末根日期派生），不写死绝对日期；③ 报告类产物另跑一次「日期一致性扫描」：全文里出现的每个 `YYYY-MM-DD` 必须能指回某个 `*_as_of` 字段。
- 判据：**机械替换的成功率只对「字面量等于今天」的串成立**，凡是「等于昨天/上一个」的串一律要人工过一遍。

- **复发实例（2026-09-25，形态是「跨市场交易日历不同步」）**：持仓检查自动化的三条腿分属三个市场，同一「日」在各腿的交易状态可以不同 —— 2026-09-25（周五）为中秋节，A 股/ETF 休市（沪深北公告 9/25–9/27，9/28 复市），**港股正常交易并完成收盘**，美股盘中。上一版脚本写的是「A 股/ETF = 2026-09-24 收盘」（当时正确），机械替换成 `20260925` 后读作「A 股/ETF = 2026-09-25 收盘」—— **一条与事实相反的价基声明，且不会有任何检查报错**。
  - 防御：复用日戳脚本前，**先查每个市场当日的开市状态**（中国看国务院放假通知 + 沪深北交易所休市公告；中国香港另查其公众假期表），再逐腿改写价基标签；三腿价基不同时必须逐腿标注，不得写成「今日全仓」。
  - 判据：**「日期」不等于「交易日」。** 机械替换只保证字面日期被改对，不保证该日期在该市场存在行情。与 D1（凭标题推时点）、D5（接口返回降序）、D9（港股取值来源）同族 —— 都是「时间维度的运行值没有指回真源」。
...
**M7 向「禁加粗」文档追加段落时，写作惯性会重新引入 `**`**（2026-09-24 建，复发 2【复发】）

- 现象：持仓检查报告体例自 2026-09-22 起禁 `**` 加粗。本轮收尾追加 §9.7（仓库未提交积压），正文里手写「产物**全部未入库**」—— 关键短语加粗是 markdown 默认习惯，**在允许加粗的文档里这么做是对的**，所以完全不触发自觉。独立写后验证器的 `无 ** 加粗` 判据当场报 FAIL（`FAIL = 1 ['无 ** 加粗']`），改成纯文本后 26/26 PASS、rc=0。
- 根因：**体例约束是文档级状态，写作习惯是全局默认**；追加段落时默认状态没有跟着该文档一起加载。与 W12（匹配串凭记忆手写）同族 —— 都是「凭默认/凭印象」覆盖了「从当前上下文取值」。
- 正确做法：① 向任何有体例约束的文档追加内容后，**必须重跑该文档的体例门禁**，不因「只加了一小段」而跳过；② 体例门禁本身要能 FAIL（本条能生效，靠的是 §3.2.1 的 known-FAIL 自检证明那两条判据不是装饰）；③ 更省事的一步：追加前先 Read 该文档头部或相邻段落，把体例抄进当次写作状态，而不是按 markdown 默认写。
- 判据：**「只加了一小段」不是跳过验证的理由** —— 追加是唯一一种「我改的位置不在我已验证过的范围内」的编辑形态。
- 附注（与 V6 配套读）：同一轮内验证器两次报 FAIL，**一次是假、一次是真**。判据是「报出的串落在产物哪一段」：落在产物的修正记录/引用段内 = 疑似假；落在新写的正文段内 = 优先信它。**不能因为上一轮是假报就默认下一轮也是假报。**

- **复发实例（2026-09-25，持仓检查自动化）**：同一份持仓检查报告体例（禁 `**`）下，当轮报告生成时**一次性引入 48 处 `**`**，由写后验证器拦下（`s.replace` 前加 `assert n%2==0`，回读确认 residual 0）。与 9/24 的区别从「追加一小段时漏带体例」变成「整篇生成时全程未加载体例」——**同一文档、连续两轮，说明体例约束从未进入生成路径，只在写后验证层被拦**。
  - 判据升级：**只靠「写后重跑体例门禁」是事后拦截，复发率不会下降**（与 W12 复发实例里那条结论同构：把损失限制在「没跑坏」，不等于「已经改好了」）。正解需要两侧都做 —— ① 体例进生成路径（模板/生成脚本头部显式声明该文档禁哪些标记，生成时就用）；② 写后门禁必跑且能 FAIL。
  - 附带风险：48 处这种量级说明「加粗」是生成侧默认行为，而非偶发手误。**凡某文档一周内两次因同一体例被拦，下一步该动的是生成端，不是继续加断言。**
...
- **聊天截图短句必须逐字核对，不得用语义近似替换原话（2026-09-24）**：将科里 17:38 的“我去，，，不要一点小事就给定情信物，hhh”误写成“不要一点小事就给它情绪价值”，把“把小礼物戏称为定情信物”的关系玩笑误读成“情绪价值”判断。正确做法：截图原话、时间和说话人三项同时核对；若用户指出原句，立即以用户确认文本为事实源，并同步修正分析中的因果解释。
...
- **复发（2026-09-20 同日晚）**：clawhub 列表接口 `GET /api/v1/skills?authorHandle=<github-user>` 返回 24 项被直接当作"我的 clawhub 清单"用了一整轮（还写进了记忆与报告），详情端点逐个核 owner 后发现其中 16 个 owner 是别人（xrowgmbh×6、zentao-cli、zerotoken、love-companion、xmemo、lunheng、wcag、doc-holmes、bankbid、ticket/franchise、powpow、sber、solidworks、github-*explorer×2、workflow-guardian 等）——**该接口的 authorHandle 参数无效，返回的是全站最近更新列表**，且 24 项的 updatedAt 恰好都是当天（我们自己发布触发的），造成"看起来就是我的"错觉。修正：真名下 22 个（含 cft 用 `?ownerHandle=` 参数核验）。规则：**列表类接口返回的归属一律不做事实源，逐条用详情端点 owner 字段核验后才能进报告/记忆**；接口返回项的时间戳高度集中时，先怀疑是"最近更新"而非"我的资产"。
...
### 3.0 十类"假成功"（两条血脉合并且为并集：2026-09-22 第 7 类 / 2026-09-23 第 8 类 / 2026-09-26 并入备份血脉两类）
...
| **`待裁定=0` / `命中 0 条`** | **托管 venv 缺依赖 → 检查整节根本没跑**（2026-09-21 备份血脉，见 §2.3 C16）
| **rc=0 + 预期标记串全绿** | **内容走 bash 双引号串内联，串内反引号被命令替换成空串，写进文件的是残缺文本**（2026-09-25 备份血脉，见 §2.2 W18）。前六类错在"动作没生效"，第 7 类错在"检查没跑"，本条错在**内容被 shell 改写而校验全部放行** |
...
- **附：自检行自身写坏的两种形态（2026-09-26，<user>e-model 每日 23:30）** —— ① **括号闭合位置错**：写成 `chk(name, can_fail(t, bad)) is False`，`is False` 落在 `chk(` 之外 → 比较结果**被丢弃**、`chk` 收到裸布尔 `False` → 自检发出**假 FAIL**，而它想守的判据本身是对的；② **判据查错字段**：断言写成「某串在 `observation` 里」，实际该串在 `evidence_summary` 里 → 同样假 FAIL。两者的共同签名 = **自检/校验报 FAIL，但人工复核被检文件是对的**。
- **判据**：见到这个签名，排查顺序固定为「先看自检行的括号闭合位置 → 再看每个判据字符串**取自哪个字段** → 最后才怀疑被检文件」。**自检机制本身是代码，代码会写坏；一个会滥 FAIL 的自检和被它守着的检查一样有害**（与本段 V3c「只会 FAIL 的闸门同样不可用」同源，区别是 V3c 错在判据近似，本条错在**表达式的结构**）。
...
### 3.2.5 验证脚本会对「产物自己引用的旧文本」假报 FAIL（V6b · 2026-09-24 建）

**产物在"修正记录"一节里引用旧措辞来描述它改了什么 —— 扫描作用域没排除这一段，就会把产物的自述当成残留错误。**

- **实证（持仓检查报告写后校验）**：`verify_report_*.py` 报 `FAIL 无残留旧时点措辞: 9/21 收盘口径`，rc=1。追查发现该串位于报告 §9.6「本轮回修」一节 —— 该节**本身就在引用这句旧措辞，说明它被改成了什么**。把扫描范围从全文收窄为 `s.split('### 9.6 本轮回修')[0]`、并加一条断言确认该段存在之后，26/26 PASS、rc=0。**被检文件始终是好的，错的是校验脚本的作用域。**
- **与 V5 同族，与 §4 第 2 条同源**：V5 是路径归一化造出假差异，本条是**作用域未排除"引用旧文本的段落"**。共同特征 —— 「产物是对的，新写的校验器是错的」，且这一族已三日多次复发（§1 榜第 9 条 4+）。
- **正确做法**：① 任何「不得残留旧措辞 / 旧值」的扫描，**必须先排除产物自身的修正记录段**；② 排除动作**必须配一条"该段存在"的断言** —— 否则段名一改，`split` 返回全文，排除动作静默失效，检查退化成 V3（永不 FAIL 的装饰）；③ 排除范围要能当面说清"为什么这一段合法"，说不清就说明扫描判据本身没设计好。
- **配套门禁（写校验器时同批加）**：每条判据附一个 **known-FAIL 样本**并**实测它真的 FAIL**（HR 47 / §3.2.1）。本轮的 `can_fail()` helper 就是为此加的，收尾输出里能看到 `PASS 自检: 标记串判据可 FAIL（known-FAIL 样本）`。
- **复发实例（2026-09-25，被检文件就是本手册自身）**：为验证 §2.2 W18 的修正是否干净，写了一条「不得再出现残缺空串」的扫描，结果 FAIL —— 追查发现唯一命中落在 **W13 自己的「现象」描述里**（W13 必须原样引用那段被 shell 啃掉的文本，才能说清问题是什么）。文件是对的，判据的作用域没排除「引用错误样本的段落」。修法沿用本条正解：先断言该段存在，再把该段切出去判残留；切后「段内 = True、段外 = False」，26 项全绿。**凡在一个专门记录错误的文件上做「不得残留某错误」的扫描，必然自撞 —— 这是本条的必然复发形态，不是偶发。**
- **附带坑（读私有真源的文件格式）**：`.workbuddy/private/` 下的 YAML **不是同一种编码** —— `instrument-master.yaml` 是 JSON 编码的 YAML，`a-share-capital-plan.yaml` / `open_items.yaml` 是真 YAML。本轮因混用解析器付出一次 `JSONDecodeError`。**读前先探首字符**，细则见 §2.1 J9。
- **V6 变体：append-only 台账文件的断言必须限定在「本次追加块」内**（2026-09-24 建，与本条同源，复发计入 §1 第 9 条）。实证：给 `UPDATES.md`（81 轮追加、187KB 的只增不改台账）追加当日记录后，校验脚本写 `assert s.count('f7a26489') == 1` —— 意图是「本次追加块里那个 md5 只出现一次」，但 `s` 是**全文**，而上一轮（09-23）条目里也写过同一个 md5 前缀 → 计数为 2 → `AssertionError`，**rc=1 而文件本身完全正确**。修法：断言前先 `s = s[s.rfind('## <当日标题>'):]` 把作用域切到本次追加块，并配一条「该标题确实存在」的断言（否则段名一改，`split` 返回全文，作用域收窄静默失效 —— 退化成 V3）。**判据：对累积型文件（UPDATES / 日志 / 台账），任何「出现 N 次」的断言都必须问一句「N 是在哪个范围内数的」。**
### 3.2.6 把「一个产物目录」当成产能的事实源（V7b · 2026-09-26 建）

**目录是产物落在哪儿，登记表是产物发没发。两者不一致时，登记表赢 —— 但前提是你得先去查登记表。**

- **实证（daily-scout 状态勘误）**：连续 5 天（2026-09-22 ~ 09-25）的简报都写「写作停滞，content/articles 最新仍 2026-09-04，已搁 21 天」，并据此推出两条结论：一条结构性判断（「扫描侧不是瓶颈，写作才是」）和一条自律条款。**两条都建在假前提上。** 查内容主表 `发布日期` 后发现：9 月公众号实际已发 9 篇，最近一篇 2026-09-17，其中就包括被反复称为「定稿待写」的那条治理选题。用 `os.walk` 扫目录 mtime 确认，2026-09-06 之后 `content/articles/` 确实没有任何写入 —— **目录没骗人，是它压根不完整**。
- **错误的三层结构**：① **单源**——把一个目录当作发布状态的唯一依据；② **把 mtime 当事实**——「没写文件」推成「没产出」，跳过了「产出可能登记在别处」；③ **静默累积**——无人复核，同一句错话连续 5 天被复制进简报、项目记忆与规则，越写越像真的。
- **与 §4 第 6 条同源**：**能测量的不许推断。** 一次 `POST /databases/{id}/query` 就能定论的事，被推断了五天。区别在于：普通推断错一次只错一次，**这句话被写进了记忆文件，于是每次会话开头都会重新加载它** —— 错的是缓存，不是某一次输出。这比单次错误贵得多。
- **判别动作（三条）**：
  1. 凡「某状态/产能/进度」的判断，**先问这个字段的权威源在哪张表**，再去查那张表。目录、日志、backlog、状态字段一律只是候选源。
  2. **目录的 mtime 只能证明「写了」不能证明「没写」。** 负向结论（停滞、无产出、未启动）必须至少两个独立源交叉，缺一个就标「待核」，不许写成结论。
  3. **任何要被写进长期记忆/规则的负向事实，写之前当场测一遍。** 记忆文件的加载频率决定了它的错误会被复制多少次。
- **配套门禁**：状态类判断落盘前，必须附一行「本结论的权威源＝<表名/字段名>，查证时间＝<日期>」。写不出这一行，就说明还没查。
- **与 V3 / V4 / V6b 的区别**：那三条都是**校验器自己出错**（永不 FAIL / 不执行 / 作用域错）。本条是**选错了事实源，且没有任何检查在看这个维度**。防线不在校验器里，在「下结论前先定权威源」这条前置动作里。
### 3.2.7 自检门限不得严于书面规则（V8 · 2026-09-26 建，与 §1 第 9 条同族）

**校验比规则严，产出的就是假 FAIL；假 FAIL 攒多了，真 FAIL 就没人信了。**

- **实证一（专有名词白名单）**：`check_2026-09-26.py` 第 6 项白名单首轮 FAIL，报三个「缺失」。追查：`Trillium` 我从未写进任何正文（白名单里多写了一个词），`Industry AI Foundry` 正文实际用的是中文词形「行业 AI Foundry」。**三个 FAIL 里两个是我自己的白名单写错，只有一个是设计好的探针。**
- **实证二（select 选项）**：`verify_2026-09-26.py` 首版判据是「select 取值必须已是线上选项」，一次 FAIL 报出 6 条。但 `GET /databases/{id}` 显示这两个属性里已堆了近 200 个一次性自由文本选项 —— **说明历史运行一直在写新选项，Notion 会照建不误**。书面规则（项目 MEMORY）只要求「≤100 字符、长描述放 children」，「必须是既有选项」是临时加的更严判据。
- **判据**：写校验前，先回答「这条 FAIL 对应哪条**写下来的**规则」。答不出，就是自造门限，要么把规则补写下来，要么把门限降回去。**把「更严」当成「更安全」是错的** —— 它买到的是噪音，付的是注意力。
- **白名单词形规则（W10 的强化）**：白名单条目必须取**正文实际使用的语言形态**。正文写「行业 AI Foundry」，白名单里就不能是 `Industry AI Foundry`；正文不用的词根本不进白名单。词形不一致的白名单**恒 FAIL**，且 FAIL 得毫无意义。
- **与 §3.2.1 的关系**：V3 说「检查要能 FAIL」，本条补后半句 —— **能 FAIL 只是必要条件，FAIL 还必须对应真缺陷**。两条合起来才是完整的检查设计约束：既不能永不 FAIL，也不能滥 FAIL。

### 3.2.8 对照探针没覆盖全部被测项，会把「探针缺项」报成「产物有残留」（V3d · 2026-09-27 建，与 V8 同族）

**一条对照的 FAIL，先问它指向探针还是指向产物 —— 手写的对照样本，恒缺项。**

- **实证（公开镜像脱敏自检）**：被测项 4 个（`<user>` / `<user>` / `<site-domain>` / `<corp>`），反向对照探针我手写成 `'C:\Users\<user>\x <site-domain> <corp>'` —— 只含 3 个。脚本输出 `[反向对照 FAIL] 探针只命中 3/4`，字面读起来像「脱敏没做干净」，实际是探针自己漏了 `<user>`。**修复动作是补探针，不是改脱敏。**
- **根因**：对照样本是从脑子里现写的，没有从被测项清单**程序化生成**。被测清单变长时，手写探针不会跟着变。
- **正确做法（三条）**：
  1. 对照探针**必须**由被测清单拼出：`probe = ' | '.join(SENSITIVE_LIST)`。清单是唯一事实源，探针是它的派生物。
  2. **把两类 FAIL 分开报**：单独一条断言「探针覆盖 N/N」，它 FAIL 说明探针坏；「替换后残留 = 空」FAIL 才说明产物坏。混在一条输出里，读到的人必然误判方向。
  3. 三向验证缺一不可 —— **输入侧**（真源确实含该串，否则脱敏无从谈起；这条常被漏掉，漏了则「输出零残留」可能只是空转）、**输出侧**（产物零残留）、**对照侧**（同一条替换链确实能把探针洗干净）。
- **与 V3 / V3c 的区别**：V3 是检查**永不 FAIL**；V3c 是判据**比规则严**（滥 FAIL）；本条是**对照物自己不全**（FAIL 得对，但归因错向）。三者共同点：错的都是校验器，不是被测产物 —— 与 §4 第 2 条同源。
- **判据**：凡是「正反成对」的检查，**反向那一半的样本必须覆盖正向的全部被测项**。覆盖不了就拆成 N 组，一组一项。

### 3.2.9 校验与执行同脚本线性排列，FAIL 不阻断后续写操作（V9 · 2026-09-27 建）

**只在脚本末尾打印「总判定」的脚本，等于没有校验 —— 它出现在所有破坏性动作之后。**

- **实证（镜像更新脚本）**：`write_mirrors_20260926.py` 线性三步 —— Step 1 脱敏三向验证（**报 1 条 FAIL**）→ Step 2 条目完整性 → Step 3 写入两个镜像工作树。`chk()` 只把失败累积进 `fails` 列表，**脚本带着 FAIL 一路执行到 Step 3，把两个文件都覆盖写了**。事后只能另写独立验证脚本补救（巧合的是备份文件是固定名，重跑整个脚本会把旧版备份覆盖掉，补救脚本必须单独写、不能复用）。
- **根因**：把「验证」和「被测动作」放进同一个脚本的同一趟线性流程，`chk()` 是**记录器**不是**闸门**。函数名叫 `chk` 容易让人以为它带拦截能力。
- **正确做法（按优先级）**：
  1. **分离脚本**：验证脚本只读，执行脚本只写。验证全绿后才运行执行脚本。这是唯一不会随重构失效的方案。
  2. 退而求其次：**第一个写操作之前**必须有硬闸门 `if fails: sys.exit(1)`。闸门位置要在动作那一行**之前**，不是之后。
  3. 判断脚本是否合格，看**第一个 `open(...,'w')` / `shutil.copy` / `os.remove` / `git push` 之前有没有 exit 判断**。没有就是不设防。
- **与本条同源的反面**：Step 3 里其实做了「写完回读校验」（`落盘一致=True`），那是**动作后**的自查，只能证明「写对了」，不能阻止「不该写」。**写前闸门和写后回验是两个东西，不能互相顶替。**
- **复发形态预判**：任何「生成 + 自检 + 落盘」三合一的脚本都会撞这条。凡是要落盘/推送/删除的脚本，先问一句「如果第 3 行的检查 FAIL 了，第 40 行的 `open('w')` 还会执行吗」。
...
- 工作区记忆：`C:\Users\<user>\WorkBuddy\.workbuddy\memory\MEMORY.md`（Skill 发布与治理、环境坑、PR 监测 engine）
- 日志：`Claw/{data-ai-daily-brief,invassistant,research,wechat-bianjieceng,tc-bigdata-marketing,work_general,shoe-advisor-mini}/.workbuddy/memory/2026-*.md`
...
**事件**：2026-09-26 23:35 本地权威文件被 `github/error-playbook-private` 镜像同步整文件覆盖（108,280 → 179,683 B），本地 Edit 沉淀静默消失且无告警。做条目级差集后定性为**两条独立演进的血脉被并成一条**，合并时丢掉了本地血脉独有的条目。

**抢救备份**：`~/.workbuddy/ERROR-PLAYBOOK.runtime-lineage-bak-20260926.md`（111,352 B，md5 `8862ea3d`）。

**本次处置（用户裁定「按全的来」）**：以本地权威版编号为基线，本地血脉独有内容全部插入正文对应小节，冲突项**两条并存**（不删任何一条），备份侧编号整体重排为不冲突的增量号。

**编号重排对照**：

| 原编号（备份血脉） | 合并后 | 内容 |
| --- | --- | --- |
| §1 第 28 条（备份） | §1 第 35 条 | 体例约束没进生成路径 |
| §1 第 25 条（备份） | §1 第 33 条 | 把「沉默」读成「安全」 |
| §1 第 27 条（备份） | §1 第 34 条 | 凭记忆手写替换/Edit 匹配串 |
| §1 第 29 条（备份） | §1 第 36 条 | 把「一个产物目录」当成产能事实源 |
| §2.1 J5（备份） | §2.1 J9 | 同名后缀不代表同一种编码 |
| §2.2 W9（备份） | §2.2 W14 | 含中文的格式化串一律 f-string |
| §2.2 W10（备份） | §2.2 W15 | 中文串专有名词被截断 |
| §2.2 W11（备份） | §2.2 W16 | 头部「上次更新」=文件内最大日期 |
| §2.2 W12（备份） | §2.2 W17 | 匹配串必须原文抄录 |
| §2.2 W13（备份） | §2.2 W18 | 反引号被 shell 命令替换 |
| §2.2 F4（权威第二个） | §2.2 F4c | 权威版内部撞号，就地改名 |
| §2.1 J7（权威第二个） | §2.1 J8 | 权威版内部撞号，就地改名 |
| §2.4 N3（权威第二个） | §2.4 N3b | 权威版内部撞号，就地改名 |
| §2.3 C11（备份） | §2.3 C16 | 托管 venv 缺依赖 → 静默 0 命中 |
| §2.3 C12（备份） | §2.3 C17 | Glob limit=100 静默截断 |
| §2.3 C13（备份） | §2.3 C18 | subprocess 调 bash 落 WSL 入口 |
| §2.3 C14（备份） | §2.3 C19 | 根目录 nul 文件使 ripgrep exit 2 |
| §2.3 C15（备份） | §2.3 C20 | 当日记忆落点不止 YYYY-MM-DD.md |
| §2.7 D10（备份） | §2.7 D16 | 登记型派生值必须可重算 |
| §2.7 D1b（备份） | §2.7 D1b | 日戳脚本不修语义标签（无冲突，原号保留） |
| §2.8 M7（备份） | §2.8 M7 | 向禁加粗文档追加段落重新引入 `**`（无冲突） |
| §3.2.4 V6（备份） | §3.2.5 V6b | 产物自述假 FAIL |
| §3.2.5 V7（备份） | §3.2.6 V7b | 产物目录当产能事实源 |
| §3.2.6 V8（备份） | §3.2.7 V8 | 自检门限不得严于书面规则（无冲突） |

**同步更新**：备份血脉正文里对旧编号的交叉引用已按上表改写；原文引用「§3.2.4 V6」的段落现指向 §3.2.5 V6b，引用「§2.3 C11」的现指向 §2.3 C16。

**防复发**：见 §1 第 31 条（W13）。镜像同步方向应为「本地权威 → 仓库镜像」，**不得反向用镜像覆盖权威文件**；收尾必须核对全文体量，只回验「我插入的串在不在」查不出整文件覆盖。
