...
| **要问用户 / 写「待定项」之前** | **先核查规则**。顺序：`~/.workbuddy/MEMORY.md` → 工作区 `.workbuddy/memory/` → 相关 skill → 本手册 §1/§2。确认「确实没有规则」才允许问；**已有规则的事一律直接执行，禁止抛回**。（2026-09-20 补：此前没有这一条，已在同一件事上被用户连续纠正两次） |
